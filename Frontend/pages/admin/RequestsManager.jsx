...
import { 
  FaClipboardList, FaSearch, FaDownload, FaUserFriends, 
  FaEye, FaCheck, FaTimes, FaTrash, FaEdit 
} from "react-icons/fa";
...
const RequestsManager = () => {
  ...
  const [preApproveTarget, setPreApproveTarget] = useState(null);
  ...
  const handleDeleteVolunteer = async () => {
    ...
    try {
      ...
      toast.success("Volunteer completely removed");
  ...
  const handleExportExcel = () => {
    const data = filteredVolunteers.map(v => ({ Name: v.fullName, Email: v.email, Phone: v.phone, Interest: v.interest }));
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Requests");
    XLSX.writeFile(wb, `Volunteer_Requests_${new Date().toLocaleDateString()}.xlsx`);
    setShowExportModal(false);
  };

  const handleExportPDF = () => {
    const doc = new jsPDF();
    doc.text("Volunteer Requests", 14, 15);
    const rows = filteredVolunteers.map(v => [v.fullName, v.email, v.phone, v.interest]);
    autoTable(doc, { head: [["Name", "Email", "Phone", "Interest"]], body: rows, startY: 20 });
    doc.save(`Volunteer_Requests_${new Date().toLocaleDateString()}.pdf`);
    setShowExportModal(false);
  };
      ...
      <div className="w-full overflow-hidden">
        {isLoadingVolunteers ? (
                ...
                {filteredVolunteers.map((vol) => (
                  <tr key={vol._id} className="hover:bg-bg/50 transition-colors group">
                    <td className="px-4 py-4 text-center">
                      <div className="w-14 h-14 rounded-full overflow-hidden border-2 border-primary/20 mx-auto bg-bg shadow-inner">
                        {vol.profilePicture ? (
                    ...
                    <td className="px-4 py-4">
                      <div className="flex justify-center items-center gap-2">
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => { setPreApproveTarget(vol); setIsTemporaryToggle(false); setShowPreApprovePopup(true); }}
                            className="flex items-center gap-2 bg-green-600 text-white px-4 py-2 rounded-lg font-bold text-sm hover:bg-green-700 transition-all shadow-md"
                          >
                            <FaCheck /> Accept
                          </button>
                          <button
                            onClick={() => handleVolunteerStatus(vol._id, "rejected")}
                            className="flex items-center gap-2 bg-blood text-white px-4 py-2 rounded-lg font-bold text-sm hover:bg-blood/90 transition-all shadow-md"
                          >
                            <FaTimes /> Reject
                          </button>
                        </div>
          ...
          <div className="py-24 text-center">
            <FaClipboardList size={64} className="mx-auto text-primary/10 mb-6" />
            <h3 className="text-2xl font-bold text-text-body mb-2">No pending requests</h3>
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-10 space-y-8">
              <div className="text-center space-y-2">
                <div className="w-20 h-20 bg-green-100 text-green-600 rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-inner"><FaCheck size={40} /></div>
                <h3 className="text-3xl font-black text-primary tracking-tight">Approve Volunteer</h3>
                <p className="text-text-body/60 font-bold uppercase tracking-widest text-xs">Assign membership type for <strong>{preApproveTarget.fullName}</strong></p>
              </div>
              <div className="grid grid-cols-1 gap-4">
                <button
                  onClick={() => setIsTemporaryToggle(false)}
                  className={`p-6 rounded-3xl border-2 text-left transition-all ${!isTemporaryToggle ? 'bg-primary/5 border-primary shadow-xl ring-2 ring-primary/10' : 'bg-white border-border hover:border-primary/20'}`}
                >
                  <div className="flex justify-between items-center mb-1">
                    <span className="font-black text-primary uppercase tracking-widest text-sm">Active Member</span>
                    {!isTemporaryToggle && <FaCheck className="text-primary" />}
                  </div>
                  <p className="text-xs text-text-body/40 font-bold">Permanent team member with full access and IDs.</p>
                </button>
                <button
                  onClick={() => setIsTemporaryToggle(true)}
                  className={`p-6 rounded-3xl border-2 text-left transition-all ${isTemporaryToggle ? 'bg-amber-50 border-amber-500 shadow-xl ring-2 ring-amber-100' : 'bg-white border-border hover:border-amber-200'}`}
                >
                  <div className="flex justify-between items-center mb-1">
                    <span className="font-black text-amber-600 uppercase tracking-widest text-sm">Temporary Member</span>
                    {isTemporaryToggle && <FaCheck className="text-amber-600" />}
                  </div>
                  <p className="text-xs text-text-body/40 font-bold">Short-term event support. Limited access duration.</p>
                </button>
              </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-white rounded-[2.5rem] shadow-xl w-full max-w-md p-10 text-center animate-in zoom-in-95 duration-300">
            <div className="w-20 h-20 bg-blood/10 text-blood rounded-3xl flex items-center justify-center mx-auto mb-6"><FaTrash size={32} /></div>
            <h3 className="text-2xl font-black text-primary tracking-tight mb-2">Permanent Deletion</h3>
            <p className="text-sm font-bold text-text-body/60 mb-8 lowercase tracking-tight">This action will erase <strong>{volunteerToDelete?.fullName}</strong> from the database forever.</p>
            <div className="flex gap-4">
              <button disabled={isDeleting} onClick={() => setShowDeleteModal(false)} className="flex-1 py-4 bg-bg rounded-2xl font-bold hover:bg-border transition-all">Go Back</button>
              <button disabled={isDeleting} onClick={handleDeleteVolunteer} className="flex-1 py-4 bg-blood text-white rounded-2xl font-black shadow-lg shadow-blood/20 hover:scale-[1.02] transition-all">
